...
// ==UserScript==
// @name         FFLogs parse & fightline for XIVAnalysis
// @version      1.0
// @description  Add a FFLogs and fightline link to a xivanalysis analyse/find
// @author       QuidamAzerty
...
$(document).ready(function () {
	...
	const fightIndex = window.location.href.split('/')[5];
	const menuContainer = $('.ui.container').find('a:nth-child(2)').get(0);
	const toAppend = $('<a></a>', {
		text: 'To FFLogs parse',
		target: '_blank',
		'class': 'item',
		href: 'https://www.fflogs.com/reports/' + reportId + (fightIndex ? '/' + fightIndex : '')
	});
	const toAppend2 = $('<a></a>', {
		text: 'To Fight Time Line',
		target: '_blank',
		'class': 'item',
		href: 'https://www.ffxiv-fightline.com/fflogs/' + reportId + (fightIndex ? '/' + fightIndex : '')
	});
	$(menuContainer).before(toAppend).before(toAppend2);
});

// ==UserScript==
// @name         XIVAnalysis & FFLogs parse for fightline
// @version      1.1
// @description  Add a xivanalysis & FFLogs parse link to fightline
// @author       QuidamAzerty
// @match        https://www.ffxiv-fightline.com/fflogs/*
// @grant        none
// @require      http://ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js
// ==/UserScript==
...
$(document).ready(function () {
	...
	const toAppend2 = $('<a></a>', {
		text: 'To xivanalysis',
		target: '_blank',
		href: 'https://xivanalysis.com/find/' + reportId + (fightIndex ? '/' + fightIndex : '')
	});

	const container = $('<div></div>', {
		style: 'position: fixed; left: 20px; bottom: 2px; width: 150px; height: 50px; background-color: white;'
	}).append(toAppend).append('<br/>').append(toAppend2);
